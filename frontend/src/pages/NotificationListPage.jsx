    ...
    return (
        ...
        >
            ...
            <div className="flex-1">
                <p
                    className={`text-sm leading-snug ${isUnread ? 'text-gray-900' : 'text-gray-500'}`}
                    style={{ whiteSpace: 'normal' }}
                >
                    <span className={isUnread ? 'font-bold' : 'font-medium'}>{notification.title}</span>
                    <span> - {notification.message}</span>
                </p>
                <span className={`text-xs ${isUnread ? 'text-blue-600 font-bold' : 'text-gray-500'}`}>
...
function NotificationListPage() {
    const { initialNotifications, pagination, unreadCount: initialUnreadCount } = useLoaderData();
    ...
    }, [isLoading, activeTab, allNotifications, unreadNotifications]);
    ...
    // --- Data Fetching Logic ---
    const fetchNotifications = async (tab, pageNum) => {
        if (isLoading) return;
        setIsLoading(true);
    ...
    const handleMarkAllAsRead = async () => {
        ...
        try {
            ...
            toast.error("Lỗi", { description: error.message || "Không thể đánh dấu đã đọc tất cả." });
        } finally {
        ...
        <main className="w-full bg-gray-100 min-h-screen">
            <div className="mx-auto max-w-3xl p-4 md:p-8">
                ...
                <div className="bg-white rounded-lg shadow-md overflow-hidden">
                    <Tabs defaultValue="all" onValueChange={handleTabChange} className="p-4">
                        ...
                        <TabsContent value="all" className="m-0 space-y-2">
                            {isLoading && allNotifications.list.length === 0 ? (
                                ...
                                allNotifications.list.map((noti, index) => (
                                    <div ref={allNotifications.list.length === index + 1 ? lastElementRef : null} key={noti._id}>
                                        <NotificationItem
                                            notification={noti}
                        ...
                        <TabsContent value="unread" className="m-0 space-y-2">
                            {isLoading && unreadNotifications.list.length === 0 ? (
                                ...
                                <p className="p-4 text-center text-sm text-gray-500">Không có thông báo chưa đọc.</p>
                            ) : (
                                unreadNotifications.list.map((noti, index) => (
                                    <div ref={unreadNotifications.list.length === index + 1 ? lastElementRef : null} key={noti._id}>
                                        <NotificationItem
                                            notification={noti}
                                            onNotificationClick={handleNotificationClick}
                                        />
