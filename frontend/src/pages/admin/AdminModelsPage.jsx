...
export default function AdminModelsPage() {
    ...
    const [selectedSort, setSelectedSort] = useState(
        `${searchParams.get("sortBy") || "brand"},${searchParams.get("sortOrder") || "asc"}`
    );
    ...
    const handleFormSubmit = async () => {
        ...
        if (!name || name.trim() === "") {
            toast.error("Lỗi Validation", { description: "Tên mẫu xe không được để trống." });
            return;
        }
        if (!brand || brand.trim() === "") {
            toast.error("Lỗi Validation", { description: "Tên thương hiệu không được để trống." });
            return;
        }
        ...
        try {
            ...
            toast.error(isEditMode ? "Lỗi khi cập nhật" : "Lỗi khi tạo mới", {
                description: error.message || `Không thể ${isEditMode ? 'cập nhật' : 'tạo'} mẫu xe.`,
            });
        } finally {
    ...
    const handleDeleteModel = async (modelId) => {
        ...
        setIsDeleting(true);
        try {
            const response = await customFetch.delete(`/admin/models/${modelId}`); // API endpoint cho model
            if (response.data.success) {
                toast.success("Mẫu xe đã được xóa thành công.");
                navigate(0);
            } else {
                throw new Error(response.data.message);
            }
        } catch (error) {
            toast.error("Lỗi khi xóa mẫu xe", {
                description: error.message || "Không thể xóa mẫu xe.",
            });
        } finally {
            setIsDeleting(false);
        }
                        ...
                        {models.length === 0 ? (
                            ...
                            models.map((model) => (
                                <TableRow key={model._id}>
                                    <TableCell>
                                        <div className="font-medium text-[#2e2e3a]">{model.name}</div>
                                        <div className="text-sm text-[#9a9aaf]">#{model._id.slice(-6).toUpperCase()}</div>
                                    </TableCell>
                                    <TableCell>{model.brand}</TableCell>
                                    <TableCell className="max-w-xs truncate">{model.description || "N/A"}</TableCell>
                ...
                <DialogContent className="sm:max-w-[600px]">
                    ...
                    <form onSubmit={(e) => { e.preventDefault(); handleFormSubmit(); }} className="grid gap-4 py-4">
                        <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor="name" className="text-right">Tên mẫu xe <span className="text-red-500">*</span></Label>
                            <Input
                                id="name"
                                name="name"
                        ...
                        <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor="brand" className="text-right">Thương hiệu <span className="text-red-500">*</span></Label>
                            <Input
                                id="brand"
                                name="brand"
                        ...
                        <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor="year" className="text-right">Năm</Label>
                            <Input
                                id="year"
                                name="year"
                                type="number"
                        ...
                        <div className="grid grid-cols-4 items-center gap-4">
                            <Label htmlFor="engine_type" className="text-right">Loại động cơ</Label>
                            <Input
                                id="engine_type"
                                name="engine_type"
                        ...
                        <DialogFooter>
                            <DialogClose asChild>
                                <Button type="button" variant="outline" onClick={() => setSelectedModel(null)}>Hủy</Button>
                            </DialogClose>
