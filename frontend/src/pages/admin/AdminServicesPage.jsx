...
export default function AdminServicesPage() {
  ...
  const handleFormSubmit = async () => {
    ...
    if (!base_price || parseFloat(base_price) <= 0) {
      toast.error("Lỗi Validation", {
        description: "Giá cơ bản phải là một số lớn hơn 0.",
      });
      return;
    }
    if (!estimated_time || parseInt(estimated_time, 10) <= 0) {
      toast.error("Lỗi Validation", {
        description: "Thời gian (phút) phải là một số lớn hơn 0.",
      });
      return;
    }
    ...
    try {
      ...
      toast.error(isEditMode ? "Lỗi khi cập nhật" : "Lỗi khi tạo mới", {
        description:
          error.message ||
          `Không thể ${isEditMode ? "cập nhật" : "tạo"} dịch vụ.`,
      });
    } finally {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <Button onClick={handleOpenAddModal}>+ Thêm dịch vụ</Button>
      </div>
