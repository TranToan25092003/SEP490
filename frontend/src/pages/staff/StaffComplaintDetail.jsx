      ...
      <div className="flex items-center">
        {[...Array(5)].map((_, i) => (
          <Star
            key={i}
            className={`h-5 w-5 ${
              i < rating ? "text-yellow-400 fill-yellow-400" : "text-zinc-300"
            }`}
          />
        ))}
      </div>
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold text-gray-800">
            Chi Tiết Khiếu Nại #{complaint._id.slice(-6)}
          </h1>
        </div>
        <div className="flex items-center gap-4">
      ...
      <div className="rounded-xl border bg-white p-6 shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-x-8 gap-y-6 sm:grid-cols-2 md:grid-cols-3">
          <DetailField label="Mã Đơn Dịch Vụ">
            {complaint.so_id
          ...
          <DetailField label="Ngày làm dịch vụ">
            {formatDate(complaint.serviceDate)}
          </DetailField>
          <DetailField label="Trạng Thái">
            {getStatusBadge(complaint.status)}
          </DetailField>
          <DetailField label="Đánh giá">
      ...
      {complaint.photos && complaint.photos.length > 0 && (
        <div className="rounded-xl border bg-white p-6 shadow-sm">
          <h2 className="text-2xl font-medium mb-6">
            Hình chụp thực tế (Từ Khách Hàng)
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {complaint.photos.map((photoUrl, index) => (
              <div
                key={index}
                className="aspect-video w-full overflow-hidden rounded-lg bg-gray-100"
              >
                <img
                  src={photoUrl}
